...
router.get('/:voter/rewards', [
    ...
    try {
        ...
        const rewards = await axios.post(
            urljoin(config.get('sdxcanUrl'), 'api/expose/rewards'),
            {
                address: voter,
                limit,
                page: page
...
router.get('/calculatingReward1Day', [], async (req, res, next) => {
    try {
        ...
        let totalSigners
        if (epoch) {
            cacheKey = urljoin(config.get('sdxscanUrl'), `api/expose/totalSignNumber/${epoch}`)
            totalSigners = cache.get(cacheKey)
            if (!totalSigners) {
                totalSigners = await axios.post(
                    urljoin(config.get('sdxscanUrl'), `api/expose/totalSignNumber/${epoch}`)
                )
    ...
    try {
        ...
        if (candidateData.rank) {
            ...
            const mnStakingEpoch = masternodeReward.multipliedBy(0.5)
                .multipliedBy(10000).dividedBy(capacity)
            ...
            const mnROI = (mnRW1Year.plus(mnStakingYear)).dividedBy(10000).multipliedBy(100).toNumber()
...
router.get('/averageroi', [], async (req, res, next) => {
    try {
        ...
        const top1MNStakingYear = masternodeReward.multipliedBy(0.5)
            .multipliedBy(10000).dividedBy(top1MN[0].capacityNumber).multipliedBy(epochYear)
        const lastMNStakingYear = masternodeReward.multipliedBy(0.5)
            .multipliedBy(10000).dividedBy(lastMN[0].capacityNumber).multipliedBy(epochYear)
        ...
        const top1MNOwnerROI = (top1MNStakingYear.plus(mnStakingYear)).dividedBy(10000).multipliedBy(100)
        const lastMNOwnerROI = (lastMNStakingYear.plus(mnStakingYear)).dividedBy(10000).multipliedBy(100)
